...
export const TaskDetail = ({ onClose = () => {}, task, onUpdate }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const updateData = {
        ...
        startDate: formData.start_date ? parseDateTimeLocal(formData.start_date) : null,
        dueDate: formData.due_date ? parseDateTimeLocal(formData.due_date) : null,
  ...
  const handleFieldBlur = (field) => {
    ...
    if (field === "start_date" || field === "due_date") {
      const originalISO = task?.[taskField] ? new Date(task[taskField]).toISOString() : "";
                  ...
                  <div>
                    {isEditing.status ? (
                      <select
                        ...
                        onBlur={() => setIsEditing((prev) => ({ ...prev, status: false }))}
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md border text-sm font-medium cursor-pointer hover:opacity-80 transition ${getStatusColor(formData.status)}`}
                      >
                        <div className="flex items-center gap-2">
                    ...
                    <div>
                      {isEditing.priority ? (
                        <select
                          ...
                          onBlur={() => setIsEditing((prev) => ({ ...prev, priority: false }))}
                          className="w-full border-2 border-indigo-500 rounded-md px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                          autoFocus
                        >
                    ...
                    <div>
                      {isEditing.startDate ? (
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Escape") {
                              setFormData((prev) => ({ ...prev, start_date: task?.start_date || "" }));
                              setIsEditing((prev) => ({ ...prev, startDate: false }));
                            }
                          }}
                        ...
                        >
                          {formData.start_date ? formatDate(formData.start_date, { withTime: true }) : "None"}
                        </button>
                      )}
                    </div>
                  </div>
                    ...
                    <div>
                      {isEditing.dueDate ? (
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Escape") {
                              setFormData((prev) => ({ ...prev, due_date: task?.due_date || "" }));
                              setIsEditing((prev) => ({ ...prev, dueDate: false }));
                            }
                          }}
                          className="w-full border-2 border-indigo-500 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                          autoFocus
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-md border border-slate-200 bg-white text-sm text-slate-700 cursor-pointer hover:bg-slate-50 transition"
                        >
                          {formData.due_date ? formatDate(formData.due_date, { withTime: true }) : "None"}
                        </button>
                      )}
                    </div>
                  </div>
